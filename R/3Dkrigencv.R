...
krige3D.ncv <- function(fun, reg.ncv, profs, cogrids, model = FALSE, krige=FALSE ,v.cutoff, v.width, v.vgm = vgm(1.5, "Gau", 10, 0), sp.cutoff, sp.width, sp.vgm = vgm(15, "Sph", 2000, 5) ) {
  ...
  for(i in 1:n.cv){
    ...
    train.results <- reg.ncv$train.results[[i]] # results obtained on training data i.e. final model fitted on training data in order to obtain the residuals for variogram fitting
    ...
    if(!model){
      ...
      var1D.list[[i]] <- qplot(dist, gamma, data = vc, geom = c("point"),color=id)+theme_bw() + 
    ...
    proj4string(res.pp)<- proj4string(cogrids)
    
    #bubble(res.pp,"residual")
    
    if(!model){
      sv <- variogram(residual~1, data=res.pp,cutoff=sp.cutoff,width= sp.width) # 480 za As
      var2D.list[[i]] <- qplot(dist, gamma, data = sv, geom = c("point"),color=id)+theme_bw() + 
        theme(legend.position="none")+theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))+
        labs(x="Distance",y="Semivariance")
    ...
    if(!model){
      ...
      var3D.list[[i]] <- qplot(dist, gamma, data = vr.def, geom = c("point"),color=id)+theme_bw() + 
        theme(legend.position="none")+theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))+
        labs(x="Distance",y="Semivariance")
    ...
    if(krige){
      ...
      proj4string(test.data) <- proj4string(cogrids)
      ...
      reg.ncv$test.results[[i]]$final.predicted <- test.data@data$final.predicted
      
    }  
    
    }
    
    out <- list(test.results = reg.ncv$test.results, var1D = var1D.list, var2D = var2D.list, var3D = var3D.list)
    return(out)
  }